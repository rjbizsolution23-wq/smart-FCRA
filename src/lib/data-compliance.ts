...
export const DATA_CATALOG: DataCollection[] = [
  ...
  { table: 'org_api_keys', store: 'd1', orgScoped: true, pii: false, purpose: 'Zapier/external API key hashes', retention: 'Until revoked + 7 years audit', backup: 'both' },
  { table: 'org_webhook_endpoints', store: 'd1', orgScoped: true, pii: false, purpose: 'Tenant outbound webhook subscriptions', retention: 'Life of integration', backup: 'both' },
  { table: 'org_webhook_deliveries', store: 'd1', orgScoped: true, pii: false, purpose: 'Webhook delivery audit log', retention: '2 years', backup: 'both' },
  { table: 'support_tickets', store: 'd1', orgScoped: true, pii: true, purpose: 'Customer service tickets with FACTS/ACTION/RESULT notes', retention: '7 years', backup: 'both' },
  { table: 'support_interactions', store: 'd1', orgScoped: true, pii: true, purpose: 'Call/SMS/email notes per ticket', retention: '7 years', backup: 'both' },
  { table: 'support_complaints', store: 'd1', orgScoped: true, pii: true, purpose: 'Formal complaint investigations', retention: '7 years', backup: 'both' },
  { table: 'support_refund_requests', store: 'd1', orgScoped: true, pii: true, purpose: 'Refund request workflow', retention: '7 years billing', backup: 'both' },
  { table: 'do_not_contact_records', store: 'd1', orgScoped: true, pii: true, purpose: 'Channel-level DNC/suppression', retention: 'Indefinite while active', backup: 'both' },
  { table: 'ai_output_reviews', store: 'd1', orgScoped: true, pii: true, purpose: 'Human review queue for AI findings/output', retention: '7 years', backup: 'both' },
  { table: 'client_billing_plans', store: 'd1', orgScoped: true, pii: false, purpose: 'CRO client subscription plan catalog', retention: '7 years billing', backup: 'both' },
  { table: 'client_invoices', store: 'd1', orgScoped: true, pii: true, purpose: 'Client subscription and PPD invoice records', retention: '7 years billing', backup: 'both' },
  { table: 'ppd_charges', store: 'd1', orgScoped: true, pii: true, purpose: 'Pay-per-delete charge queue and approval', retention: '7 years billing', backup: 'both' },
  { table: 'escalation_queue', store: 'd1', orgScoped: true, pii: true, purpose: 'Round-2 / MOV escalation recommendations', retention: 'Case life / litigation', backup: 'both' },
  { table: 'client_progress_reports', store: 'd1', orgScoped: true, pii: true, purpose: 'Monthly progress PDF/email summaries', retention: '7 years', backup: 'both' },
  { table: 'marketing_campaigns', store: 'd1', orgScoped: true, pii: false, purpose: 'Email/SMS campaign definitions and stats', retention: '2 years', backup: 'both' },
  { table: 'campaign_deliveries', store: 'd1', orgScoped: true, pii: true, purpose: 'Per-recipient campaign delivery log', retention: '2 years', backup: 'both' },
  { table: 'consent_evidence', store: 'd1', orgScoped: true, pii: true, purpose: 'Litigation-grade consent records by channel/purpose', retention: '7 years', backup: 'both' },
  { table: 'communication_attempts', store: 'd1', orgScoped: true, pii: true, purpose: 'Pre-send gate audit (ALLOW/BLOCK) per lane', retention: '7 years', backup: 'both' },
  { table: 'crm_workflow_runs', store: 'd1', orgScoped: true, pii: true, purpose: 'Active lifecycle/workflow instances', retention: '2 years', backup: 'both' },
  { table: 'crm_workflow_steps', store: 'd1', orgScoped: true, pii: false, purpose: 'Scheduled workflow step queue', retention: '2 years', backup: 'both' },
  { table: 'client_tasks', store: 'd1', orgScoped: true, pii: false, purpose: 'Consumer task center items', retention: 'Case life', backup: 'both' },
  { table: 'staff_action_queue', store: 'd1', orgScoped: true, pii: true, purpose: 'Next-best-action staff queue', retention: '1 year', backup: 'both' },
  { table: 'automation_definitions', store: 'd1', orgScoped: true, pii: false, purpose: 'Custom WHEN/IF/THEN automation definitions', retention: '2 years', backup: 'both' },
  { table: 'campaign_approval_log', store: 'd1', orgScoped: true, pii: false, purpose: 'Campaign approval audit trail', retention: '7 years', backup: 'both' },
  { table: 'communication_preferences', store: 'd1', orgScoped: true, pii: true, purpose: 'Per-client service vs marketing channel prefs', retention: 'Case life', backup: 'both' },
  { table: 'client_timeline_events', store: 'd1', orgScoped: true, pii: true, purpose: 'Unified client activity timeline', retention: '7 years', backup: 'both' },
  { table: 'push_subscriptions', store: 'd1', orgScoped: true, pii: true, purpose: 'Web push subscription endpoints', retention: 'Case life', backup: 'both' },
  { table: 'call_log', store: 'd1', orgScoped: true, pii: true, purpose: 'Outbound/inbound call audit with disclosure flags', retention: '7 years', backup: 'both' },
  { table: 'ghl_webhook_events', store: 'd1', orgScoped: true, pii: true, purpose: 'Inbound GHL webhook processing log', retention: '1 year', backup: 'both' },
  { table: 'integration_secrets', store: 'd1', orgScoped: true, pii: true, purpose: 'Encrypted integration credential vault', retention: 'Until revoked', backup: 'none' },
  { table: 'integration_connections', store: 'd1', orgScoped: true, pii: false, purpose: 'Per-org integration connection health/status', retention: '2 years', backup: 'both' },
  { table: 'external_identity_links', store: 'd1', orgScoped: true, pii: true, purpose: 'Master Consumer ID external system mappings', retention: 'Case life', backup: 'both' },
  { table: 'identity_resolution_queue', store: 'd1', orgScoped: true, pii: true, purpose: 'Ambiguous identity match manual review', retention: '1 year', backup: 'both' },
  { table: 'platform_events', store: 'd1', orgScoped: true, pii: true, purpose: 'Event bus for workflows and integrations', retention: '2 years', backup: 'both' },
  { table: 'event_idempotency_keys', store: 'd1', orgScoped: true, pii: false, purpose: 'External event deduplication', retention: '90 days', backup: 'both' },
  { table: 'integration_job_queue', store: 'd1', orgScoped: true, pii: false, purpose: 'Integration retry / dead-letter queue', retention: '90 days', backup: 'both' },
  { table: 'maker_checker_requests', store: 'd1', orgScoped: true, pii: true, purpose: 'Dual-approval sensitive actions', retention: '7 years', backup: 'both' },
  { table: 'integration_access_log', store: 'd1', orgScoped: true, pii: false, purpose: 'Credential vault access audit', retention: '7 years', backup: 'both' },
...
export async function resolveOrgDeviceLockPolicy(db: SessionDb, orgId: string): Promise<DeviceLockPolicy> {
  try {
    ...
    return { enabled: false, maxDevices: 0 };
  }
}
