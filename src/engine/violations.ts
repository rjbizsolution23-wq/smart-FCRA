...
function checkInquiryViolations(inquiries: ParsedInquiry[]): Violation[] {
  ...
  if (last6mo.length > 6) {
    violations.push({
      ...
      evidence: `${last6mo.length} hard inquiries detected in the last 6 months: ${last6mo.map(i => i.creditorName).join(', ')}. Elevated volume warrants permissible-purpose verification — not automatic proof of unauthorized access.`,
      explanation: `An elevated inquiry count may indicate unauthorized pulls, identity theft, or aggressive lending. Each inquiry must be fact-checked against permissible purpose before asserting a §1681b violation. Marked for attorney review.`,
      caseLaw: 'Patel v. Trans Union, LLC, 2018 WL 3062948 (N.D. Ga.)  -  each unauthorized inquiry is a separate FCRA violation (requires proven lack of permissible purpose)',
