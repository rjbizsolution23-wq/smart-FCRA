...
4. Re-run **Smart FCRA v2 — PR Preview Deploy** on the open PR.

Until both secrets exist, preview workflows skip Pages deploy and comment on the PR instead of failing unit tests. Production/hotfix deploys on `main` fail loudly if the secrets are missing.
